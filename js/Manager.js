    ...
    init(){
        (function(win, $) {
            ...
            $(win).on("resize", function() {
                ...
                const paddingHeight = (parseFloat($("body").css("height")) - parseFloat($("#field").css("height")) - (2 * parseFloat($("#field").css("border-width"))) )/2;
...
class GameManager{
    ...
    setUpTheGame(){
        ...
        for(let i = this.#howManyPlayer; i>0; i--){
            $("#itemDownPadding").append($(`<div id='item_description_${i}' class='item_description'></div>`));
        }
    ...
    loopWhileGameIsExecuting = ()=>{
        ...
        this.#HEROMANAGER.heroProjectionMove(this.#MONSTERMANAGER.MONSTERLIST, this.#COMMONOBSTACLE, this.#HEROOBSTACLE);
        this.#HEROMANAGER.heroMove(this.#COMMONOBSTACLE, this.#HEROOBSTACLE);
        this.#HEROMANAGER.heroAttack(this.#MONSTERMANAGER.MONSTERLIST, this.#MONSTEROBSTACLE);
        
        this.#HEROMANAGER.heroDeathCheck();

        this.#MONSTERMANAGER.monsterMoveAnimation();
        this.#MONSTERMANAGER.monsterSpecialAttack(this.#HEROMANAGER.HEROLIST, this.#MONSTERMANAGER.MONSTERLIST, this.#COMMONOBSTACLE, this.#MONSTEROBSTACLE);
        this.#MONSTERMANAGER.monsterWalkAround(this.#COMMONOBSTACLE, this.#MONSTEROBSTACLE);
